...
"The world is posited as real, not constituted by the experience of the individual observer, and shared" — this
is kept. Its ground is **intersubjectivity** (not only I but others converge on the same world, and the world
pushes back), which is a **practical warrant for treating solipsism as methodologically unhelpful — not a
refutation of it.**
...
⚠️ **Corrected v1.18-draft.** *"Prior to observers"* was ambiguous between temporal priority and independence;
the latter is what the methodological posit needs. The temporal reading would collide needlessly with F11 and the
`τ` / time-index separation at O8. Likewise, convergence and resistance are practical grounds for the posit, not a
logical refutation of solipsism. The weaker wording changes no operational commitment of F2.
...
planned. One line is held: **attribution is not desert.** That A→R is repaid along V is blocked, and
belongs to C4's reconciliation side (**corrected v1.16**: what blocks it is not Wall 3 but the **refusal of
P6**. Wall 3 is a descriptive fact about natural law and does not reach readings that place the settlement
outside the observation window — see the C4 entry in F0). "The deed is there" and "the deed is repaid" are
different claims, and neither F7 nor the block delivers the second.
...
These are not problems to "solve." The positions are **strongly favoured** by the foundation above — motivated by it, though not strictly entailed (each is a defeasible best-fit, not a theorem).
...
### v1.16 — 2026-08-02 · where the load actually sits, and the direction of a borrowed formula

The three items v1.15 deferred. None moves a position; each rewrites **where the load sits** in an argument.

- **Load transferred from Wall 3 to P6.** Wall 3 is, by v1.11's grading, a **descriptive fact about natural
  law**, so it blocks only the **naturalistic** version of C4. Theodicy, the karmic ledger and Kant's postulates
  all place the settlement outside the observation window by design — they agree with Wall 3, and the postulate
  is placed precisely because nature does not proportion. What actually blocks the traditional versions is the
  **refusal of P6**, backed by **F3**'s two-way silence. Both the C4 entry in F0 and **F12 §3** were leaning on
  Wall 3; both are corrected.
- **v1.8's addressee argument demoted from independent ground to corroboration.** "Repayment needs an addressee"
  is not argued. A bearerless continuum (santāna) grants the premise — the doer is not found — and answers that
  fruition requires not an addressee but **continuity**. And the objection does not only come from outside:
  **F12's "neighbourhood" note names santāna as the nearest form to this theory's own**, and a style meeting
  §2's three conditions is itself a candidate addressee. The same objection regenerates from the corpus's own
  vocabulary. **The compulsion stands but its route is replaced** — completeness requires the carrier to cross
  death, and no means of supplying that survives while **F3** is kept. The modal strength is unchanged; only
  what v1.9's index is relative to moves, **from F12 to F3**. *A sentence rewritten four times turns out to have
  been a branch rather than the trunk. That is recorded too.*
- **Three lines added at C9's "what is not borrowed".** **(i)** P6 is not smuggling — it is open positing, and
  the treatment given Kant's row is owed here too. **(ii)** The direction of the borrowed formula in its source
  is probably the reverse of its direction here — the Visuddhimagga verse answers "if there is no self, whose
  kamma?", where no-self is deployed to *preserve* fruition. The lineage caution guarded **provenance** but not
...
### v1.18-draft — 2026-08-14 · F0-L's theorem premises and F2's posit are stated at their actual strength

- **F0-L's effective-axiomatization premise is now in C, where the Gödel step uses it.** The former text used
  that premise in the derivation and omitted it from A–D, then concluded with the bare four-letter formula. The
  conclusion now explicitly carries the corrected C. The Tarski route is split off: it needs a definable, total
  truth predicate for E's own language and is not smuggled into the Gödel branch.
- **The independence examples are demoted pending rederivation.** Adding effectiveness to C changes the claim
  that each condition can independently be dropped. The joint theorem remains; the stronger claim that silence is
  the only exit does not remain frozen pending the declared rederivation in `20-open.md`.

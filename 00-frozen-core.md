...
Gödel's first incompleteness theorem gives, for any consistent (**A**), effectively-axiomatized system that interprets arithmetic (**C**), a sentence it cannot decide; **B** makes that sentence — a fact about the system itself — one the description must cover, so completeness (**D**) fails on it (and, reading C1 as "E contains its own truth predicate", Tarski's undefinability theorem collapses **A**). Hence **A + B + C + D cannot hold together**. Each demand is independently necessary — drop any one and the rest are jointly satisfiable: drop **D** → consistent but incomplete description (exactly this theory's F4/R3); drop **C** → complete but self-blind systems (Presburger arithmetic); drop **B** → the scope no longer has to include the system's own facts, so a complete, consistent description *from outside* becomes possible (Platonism, the God's-eye seat); drop **A** → paraconsistency. This is an Arrow-style impossibility: individually reasonable demands, jointly unsatisfiable. For *this corner*, "give up completeness → stay silent about the remainder" is not a stylistic preference; it is the only exit that preserves consistency, self-inclusion and expressiveness.
